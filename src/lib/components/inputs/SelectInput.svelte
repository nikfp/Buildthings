<script lang="ts">
	...
	export let addNew: { buttonText: string; onSelected: () => any | Promise<any> } | undefined =
		undefined;
</script>
...
<div class="select-wrapper">
	<ValidationMessage for={name} let:messages={message}>
		{#if addNew}
			<Button size="small" type={'button'} on:click={() => addNew?.onSelected()}
				>{addNew.buttonText}</Button
			>
		{/if}
		<label class={`select-label ${message ? 'select-warning' : ''}`} for={name}
			>{title ?? name}</label
		>
		<select
			data-test="hello"
			bind:value={selected}
			{name}
			class={`select ${message ? 'select-warning' : ''}`}
		>
			<option value="" disabled>{inputMessage}</option>
			{#each values as value (value.key)}
				{#if value.key === selected}
					<option value={value.value} selected>{value.optionText}</option>
				{:else}
